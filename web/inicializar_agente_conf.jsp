<%-- 
    Document   : inicializar_agente_conf
    Created on : 04/12/2014, 02:27:15
    Author     : vinicius
--%>
...
<%
    ...
    idIni = request.getParameter("idIni");
    idAgente = Integer.parseInt(request.getParameter("idAgente"));
    op = request.getParameter("op");
    status = request.getParameter("status");

    Agente ag = AplCadastrarAgente.obterAgente(idAgente);
    cor = InicializarAgente.obterIA(Integer.parseInt(idIni)).getCor();
    forma = InicializarAgente.obterIA(Integer.parseInt(idIni)).getForma();
    tamanho = InicializarAgente.obterIA(Integer.parseInt(idIni)).getTamanho();
%>
<!DOCTYPE html>
<html>
    <head>
        ...
        <script language="JavaScript" type="text/JavaScript">
            ...
            {
                window.location.href='editarexp.jsp?pai=inicializar_agente_conf&expr=' + encode(expressao) + '&var=' + idVar + '&idIni=<%=idIni%>'+ '&agente=<%=idAgente%>';
            }
            
            function alterar(idVar, expressao)
            {
                window.location.href='EditorExpressao?pai=inicializar_agente_conf&idIni=<%=idIni%>&agente=<%=idAgente%>&var='+idVar+'&expr='+expressao;
            }
        </script>
        <title>Configurar Agente</title>
    </head>
    <body>
        <form action="controlador.ControladorWeb" method="post" name="acCriar" id="acCriar">
            <table  width="500" border="1" bgcolor="#BBD0BF">
                <tr> <td colspan="3"><div align="center" class="style38"> Inicialização do Agente</div></td> </tr>
                <%
                    ...
                    for (int k = 0; k < variaveis.size(); k++) {
                        ...
                        String valor = InicializarAgenteVariaveis.obterIAV(Integer.parseInt(idIni), idVar).getValor();
                %>
                <tr> 
                    <td> <%=nomeVar%> </td>
                    <%
                ...
                <tr>
                    <td colspan="4"><div align="center">
                            ...
                            <input name="op" type="hidden" value="variaveis">
