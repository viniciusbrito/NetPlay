<%-- 
    Document   : acao_conf_criar.jsp
    Created on : 27/10/2014, 22:57:52
    Author     : vinicius
--%>
...
<%
    ...
    idAcao = request.getParameter("idAcao");
    ...
    cor = GerarNovoObjeto.buscar(Integer.parseInt(idAcao), idAgente).getCor();
    posicao = GerarNovoObjeto.buscar(Integer.parseInt(idAcao), idAgente).getPosicao();
    
    Agente ag = AplCadastrarAgente.obterAgente(idAgente);
    if(ag.getTipo().equals("Movel")) {
        forma = GerarNovoObjeto.buscar(Integer.parseInt(idAcao), idAgente).getForma();
        tamanho = GerarNovoObjeto.buscar(Integer.parseInt(idAcao), idAgente).getTamanho();
    }
...
<html>
    <head>
        ...
        <script language="JavaScript" type="text/JavaScript">
            ...
            function expressao(idVar, expressao)
            {

            window.location.href='editarexp.jsp?pai=acao_criar_conf&expr=' + encode(expressao) + '&var=' + idVar + '&acao=<%=idAcao%>'+ '&agente=<%=idAgente%>';
            }
            
            function alterar(idVar, expressao)
            {
                window.location.href='EditorExpressao?pai=acao_criar_conf&idAcao=<%=idAcao%>&agente=<%=idAgente%>&var='+idVar+'&expr='+expressao;
            }
            
        </script>
        <title>Configurar Agente</title>
    </head>
    <body>
        <form action="controlador.ControladorWebCCA" method="post" name="acCriar" id="acCriar">
            <table  width="500" border="1" bgcolor="#BBD0BF">
                <tr> <td colspan="3"><div align="center" class="style38"> Configuração do Agente ao ser criado</div></td> </tr>
                <%
                    List variaveis = AplCadastrarVariavel.variaveisDoAgente(idAgente);
                    String varString = "";
                    for (int k = 0; k < variaveis.size(); k++) {
                        Variavel var = (Variavel) variaveis.get(k);
                        String nomeVar = var.getNome();
                        int idVar = var.getIdVariavel();
                        String valor = AplCadastrarAcao.buscarVarCriar(Integer.parseInt(idAcao), idVar);
                %>
                <tr> 
                    <td> <%=nomeVar%> </td>
                    <%
                        if (var.getTipo().equals("Valor")) {
                    %><td><input  name="<%=idVar%>" type="text" readonly value="<%=valor%>"/></td> 
                    <td bgcolor="#333300">
                        <div align="center" class="style34">
                            <a href="javascript:expressao(<%=idVar%>, '<%=valor%>')">Editar Express&atilde;o</a>
                        </div>
                    </td> 
                </tr>
                <%  }
                    ...
                    if (var.getTipo().equals("Logica")) {
                        VariavelLogica varValor = (VariavelLogica) variaveis.get(k);
                        Boolean valorLogico = Boolean.parseBoolean(valor);
                %><td>
                    <select name="<%=idVar%>" onChange="javascript:alterar(<%=idVar%>, this.value)">
                        <%if (valorLogico == true) {%>
                        <option selected value="true">Verdadeiro</option>
                        <%} else {%>
                ...
                <%          }
                        varString += Integer.toString(idVar) + ";";
                    }
                %>
                <tr>
                    <td>Cor</td>
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="tamanho">
                                <%
                                    for(int z = 1; z <= 5; z ++) {%>
                                        <option <%= ((tamanho == z) ? "selected" : " ") %> value="<%=z%>"><%=z%></option>
                                <%  } %>
                            </select>
                        </td>
                    </tr>
