...
<%
String expressao;
String agente; //id do agente
String id; //id da condicao
String numAgentes;
...
for (int i=0; i<condicoes.size(); i++)
{
    ...
    if(condicao.getTipo().equals("Per"))
    {
        ...
        agPrinc = percepcao.getAgentePrincipal();
        break;
    }
    if(condicao.getTipo().equals("Pos") && 
            condicao.getIdCondicao().intValue() != Integer.parseInt(id))
    {
        Posicionamento pos = (Posicionamento) condicao;
        if(pos.getAgentePrincipal() != null)
        {
            agPrinc = pos.getAgentePrincipal();
            break;
        }
    }
}
  ...
  function encode(s){
       ...
       function link()
       {
            if(document.posicionamento.numAgentes.value == 2)
                window.location.href='editarexp.jsp?pai=altcondpos&agente=' + document.posicionamento.agente.value + '&expr=' + encode('<%= expressao%>') + '&agentePrincipal='+ document.posicionamento.agentePrincipal.value + '&numAgentes=' + document.posicionamento.numAgentes.value + '&idComposicao=<%=idComposicao%>&id=<%=id%>'
            else
                window.location.href='editarexp.jsp?pai=altcondpos&expr=' + encode('<%= expressao%>') + '&agente='+ document.posicionamento.agente.value + '&numAgentes=' + document.posicionamento.numAgentes.value + '&idComposicao=<%=idComposicao%>&id=<%=id%>'
       }
//-->
</script>
</head>
